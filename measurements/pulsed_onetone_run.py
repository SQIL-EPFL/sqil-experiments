"""
Pulsed onetone with readout pulse frequency mixing.
readout - SHFQC x external LO source
qubit drive - SHFQC
"""
...
####################################################################################
LO_source = "SignalCore"  # "SGS" or "SignalCore"
Yokogawa = False  # True or False
####################################################################################

exp_name = "pulsed_onetone"
tags = ["0_pulsed_onetone"]

# define readout frequency list
freq_start = param_dict["pulsed_onetone"]["ro_freq_start"]
freq_stop = param_dict["pulsed_onetone"]["ro_freq_stop"]
if param_dict["pulsed_onetone"]["ro_freq_npts"] == False:
    freq_npts = (
        int((freq_stop - freq_start) / param_dict["pulsed_onetone"]["ro_freq_step"]) + 1
    )
else:
    freq_npts = param_dict["pulsed_onetone"]["ro_freq_npts"]
param_dict["pulsed_onetone"][
    "ro_freq_npts"
...
local_param_list = param_dict["pulsed_onetone"].keys()
for key in local_param_list:
    if key in param_dict.keys():
        if not param_dict["pulsed_onetone"][key] == False:
            param_dict[key] = param_dict["pulsed_onetone"][key]
...
if param_dict["pulsed_onetone"]["sweep"] == False:
    ...
    datadict = DataDict(
        ...
        data=dict(axes=["ro_freq"]),
    )
    datadict.validate()
else:
    exp_name = exp_name + "_vs_" + param_dict["pulsed_onetone"]["sweep"]
    tags.append(f"0_{exp_name}")
    param_dict[param_dict["pulsed_onetone"]["sweep"]] = "sweeping"
    ...
    datadict = DataDict(
        ...
        data=dict(axes=["ro_freq", "sweep_param"]),
    )
    datadict.validate()
...
with DDH5Writer(datadict, db_path_local, name=exp_name) as writer:
    ...
    for sweep_param in tqdm.tqdm(sweep_list):
        # update param_dict
        if not param_dict["pulsed_onetone"]["sweep"] == False:
            param_dict[param_dict["pulsed_onetone"]["sweep"]] = sweep_param

        if LO_source == "SGS":
            sgsa.power(param_dict["ro_exLO_power"])
    ...
    if param_dict["pulsed_onetone"]["sweep"] == False:
        ...
        fig_fit.savefig(
            f"{writer.filepath.parent}/00_1D_vs_ro_freq_fit.png", bbox_inches="tight"
        )
        writer.save_text(
...
if param_dict["pulsed_onetone"]["sweep"] == "ro_power":
    if not param_dict["pulsed_onetone"]["ro_freq_npts"] == 1:
