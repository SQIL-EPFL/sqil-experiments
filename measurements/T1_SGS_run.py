...
from T1_SGS_exp import exp_file, main_exp

exp_name = "T1_SGS"
tags = ["0_T1_SGS"]

# define pulse length list
if param_dict["T1"]["logsweep"] == True:
    delay_sweep = param_dict["T1"]["delay_sweep_list"]
else:
    delay_sweep = np.linspace(
        param_dict["T1"]["delay_sweep_start"],
        param_dict["T1"]["delay_sweep_stop"],
        param_dict["T1"]["delay_sweep_npts"],
    )

# update param_dict when local parameter used
local_param_list = param_dict["T1"].keys()
for key in local_param_list:
    if key in param_dict.keys():
        if not param_dict["T1"][key] == False:
            param_dict[key] = param_dict["T1"][key]

if param_dict["T1"]["sweep"] == False:
    sweep_list = [0]
    # define DataDict for saving in DDH5 format
    datadict = DataDict(
        readout_delay=dict(unit="sec"), data=dict(axes=["readout_delay"])
    )
    datadict.validate()
else:
    exp_name = exp_name + "_vs_" + param_dict["T1"]["sweep"]
    tags.append(f"0_{exp_name}")
    param_dict[param_dict["T1"]["sweep"]] = "sweeping"
    ...
    datadict = DataDict(
        ...
        data=dict(axes=["readout_delay", "sweep_param"]),
    )
    datadict.validate()
...
with DDH5Writer(datadict, db_path_local, name=exp_name) as writer:
    ...
    for sweep_param in tqdm.tqdm(sweep_list):
        # update param_dict
        if not param_dict["T1"]["sweep"] == False:
            param_dict[param_dict["T1"]["sweep"]] = sweep_param

        ## update parameters
        # setting of SignalCore
...
if param_dict["T1"]["sweep"] == False:
    ...
    plt.savefig(f"{writer.filepath.parent}/T1_inspect.png", bbox_inches="tight")
    inspection_only_optm.inspect_decaying_exponent(delay_sweep, data, figure=None)
    plt.savefig(f"{writer.filepath.parent}/00_T1_inspect.png", bbox_inches="tight")
