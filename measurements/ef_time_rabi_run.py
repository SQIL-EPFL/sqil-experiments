...
from ef_time_rabi_exp import exp_file, main_exp
from inspection import inspect_decaying_oscillations

exp_name = "ef_Time_Rabi"
tags = ["0_ef_timerabi"]

# define sweep list (ef pulse length)
ef_pulse_length_start = param_dict["ef_time_rabi"]["ef_pulse_length_start"]
ef_pulse_length_stop = param_dict["ef_time_rabi"]["ef_pulse_length_stop"]
if param_dict["ef_time_rabi"]["ef_pulse_length_npts"] == False:
    ef_pulse_length_npts = (
        int(
            (ef_pulse_length_stop - ef_pulse_length_start)
            / param_dict["ef_time_rabi"]["ef_pulse_length_step"]
        )
        + 1
    )
else:
    ef_pulse_length_npts = param_dict["ef_time_rabi"]["ef_pulse_length_npts"]
param_dict["ef_time_rabi"][
    "ef_pulse_length_npts"
] = ef_pulse_length_npts  # update ef_pulse_length_npts to param_dict
ef_pulse_length_sweep = np.linspace(
    ef_pulse_length_start, ef_pulse_length_stop, ef_pulse_length_npts
)

# update param_dict when local parameter used
local_param_list = param_dict["ef_time_rabi"].keys()
for key in local_param_list:
    if key in param_dict.keys():
        if not param_dict["ef_time_rabi"][key] == False:
            param_dict[key] = param_dict["ef_time_rabi"][key]

if param_dict["ef_time_rabi"]["sweep"] == False:
    sweep_list = [0]
    ...
    exp_name = exp_name + "_vs_" + param_dict["ef_time_rabi"]["sweep"]
    tags.append(f"0_{exp_name}")
    param_dict[param_dict["ef_time_rabi"]["sweep"]] = "sweeping"
    ...
    datadict = DataDict(
        ...
        data=dict(axes=["ef_pulse_length", "sweep_param"]),
    )
    datadict.validate()
...
with DDH5Writer(datadict, db_path_local, name=exp_name) as writer:
    ...
    for sweep_param in tqdm.tqdm(sweep_list):
        # update param_dict
        if not param_dict["ef_time_rabi"]["sweep"] == False:
            param_dict[param_dict["ef_time_rabi"]["sweep"]] = sweep_param
        ...
        if param_dict["ef_time_rabi"]["sweep"] == False:
            writer.add_data(
                ef_pulse_length=ef_pulse_length_sweep,
                data=data,
            )
...
inspect_decaying_oscillations(ef_pulse_length_sweep, data, figure=None)
plt.savefig(f"{path}/ef_time_rabi.png", bbox_inches="tight")
