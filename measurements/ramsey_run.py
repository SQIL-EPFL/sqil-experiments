...
path = str(filepath_parent)
if param_dict["ramsey"]["sweep"] == False:
    if param_dict["ramsey"]["fit_2sin"] == True:
        Two_sine_fit.inspect_decaying_2oscillations(interval_sweep, data * 1e3)
        plt.savefig(
            f"{writer.filepath.parent}/ramsey_inspect_2sinfit.png", bbox_inches="tight"
        )
    else:
        ...
        inspection_only_optm.inspect_decaying_oscillations(
            interval_sweep, data, figure=None
        )
        plt.savefig(
            f"{writer.filepath.parent}/00_ramsey_inspect.png", bbox_inches="tight"
        )
