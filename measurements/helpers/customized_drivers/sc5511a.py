...
class Device_rf_params_t(ctypes.Structure):
    _fields_ = [
        ("rf1_freq", ctypes.c_ulonglong),  # current ch#1 rf frequency
...
class Operate_status_t(ctypes.Structure):
    _fields_ = [
        ...
        (
            "ref_out_select",
            ctypes.c_ubyte,
        ),  # indicates the reference output select: 0 = 10 MHz, 1 = 100 MHz
        (
            "list_mode_running",
            ctypes.c_ubyte,
        ),  # indicates list/sweep is triggered and currently running
        (
            "rf1_mode",
            ctypes.c_ubyte,
...
class Pll_status_t(ctypes.Structure):
    _fields_ = [
        ("sum_pll_ld", ctypes.c_ubyte),  # lock status of main pll loop
        (
            "crs_pll_ld",
            ctypes.c_ubyte,
        ),  # lock status of coarse offset pll loop (used only for harmonic mode)
        ("fine_pll_ld", ctypes.c_ubyte),  # lock status of the dds tuned fine pll loop
        (
            "crs_ref_pll_ld",
            ctypes.c_ubyte,
        ),  # lock status of the coarse reference pll loop
        (
            "crs_aux_pll_ld",
            ctypes.c_ubyte,
        ...
        (
            "ref_10_pll_ld",
            ctypes.c_ubyte,
        ),  # lock status of the master 10 MHz TCXO pll loop
        ("rf2_pll_ld", ctypes.c_ubyte),
    ]  # lock status of the chn#2 pll loop
...
class List_mode_t(ctypes.Structure):
    _fields_ = [
        (
            "sss_mode",
            ctypes.c_ubyte,
        ),  # 0 uses list for buffer, 1 calculates using stop-start-step
        (
            "sweep_dir",
            ctypes.c_ubyte,
        ),  # 0 start/beginning to stop/end, 1 stop/end to start/beginning
        ("tri_waveform", ctypes.c_ubyte),  # 0 sawtooth, 1 triangular
        (
            "hw_trigger",
            ctypes.c_ubyte,
        ),  # 0 soft trigger expected, 1 hard trigger expected
        (
            "step_on_hw_trig",
            ctypes.c_ubyte,
        ...
        (
            "return_to_start",
            ctypes.c_ubyte,
        ),  # if 1, frequency returns to start frequency after end of cycle(s)
        (
            "trig_out_enable",
            ctypes.c_ubyte,
        ),  # 1 enables a trigger pulse at the trigger on pin
        ("trig_out_on_cycle", ctypes.c_ubyte),
    ]  # 0 trigger out on every frequency change, 1 trigger on cycle complete


class Device_status_t(ctypes.Structure):
    _fields_ = [
        ("list_mode", List_mode_t),  # list mode parameters
...
class Clock_config_t(ctypes.Structure):
    _fields_ = [
        (
            "ext_ref_lock_enable",
            ctypes.c_ubyte,
        ),  # select to lock to either external reference source
...
class SignalCore_SC5511A:
    def __init__(
        ...
        name: str,
        serial_number: str,
        dll_path: str = "C:\\Program Files\\SignalCore\\SC5511A\\api\\c\\x64\\sc5511a.dll",
        debug=False,
        **kwargs: Any,
    ):
    ...
    def get_output(self):
        ...
        """
        error_code = self._dll.sc5511a_get_device_status(
            self._handle, ctypes.byref(self._device_status)
        )
    ...
    def get_frequency(self):
        ...
        error_code = self._dll.sc5511a_get_rf_parameters(
            self._handle, ctypes.byref(self._rf_params)
        )
    ...
    def set_clock_reference(self, ref, direct_lock, high_, lock_to_external):
        ...
        error_code = self._dll.sc5511a_set_clock_reference(
            self._handle, ext_ref, ext_direct_lock, high, lock
        )
    ...
    def get_ext_ref_lock_enable(self):
        ...
        error_code = self._dll.sc5511a_get_clock_config(
            self._handle, ctypes.byref(self._clock_config)
        )
    ...
    def get_alc_dac(self) -> int:
        ...
        error_code = self._dll.sc5511a_get_alc_dac(
            self._handle, ctypes.byref(self._dac_value)
        )
    ...
    def get_signal_phase(self):
        error_code = self._dll.sc5511a_get_signal_phase(
            self._handle, ctypes.byref(self._phase)
        )
    ...
    def get_temperature(self):
        ...
        error_code = self._dll.sc5511a_get_temperature(
            self._handle, ctypes.byref(self._temperature)
        )
    ...
    def get_idn(self) -> Dict[str, Optional[str]]:
        ...
        error_code = self._dll.sc5511a_get_device_info(
            self._handle, ctypes.byref(self._device_info)
        )
        ...
        IDN: Dict[str, Optional[str]] = {
            ...
            "manufacture_date": "20{}={}-{}".format(
                self._device_info.manufacture_date.year,
    ...
    def reg_read(self, reg_byte, ins_word):
        ...
        error_code = self._dll.sc5511a_reg_read(
            self._handle, reg_byte, instruct_word, ctypes.byref(self._reg_read)
        )
    ...
    def set_synth_mode(self, disable_spur_suppress=0, low_loop_gain=0, lock_mode=0):
        ...
        """
        error_code = self._dll.sc5511a_set_synth_mode(
            self._handle, disable_spur_suppress, low_loop_gain, lock_mode
        )
    ...
    def list_buffer_read(self, address=0) -> int:
        ...
        error_code = self._dll.sc5511a_list_buffer_read(
            self._handle, address, ctypes.byref(self._freq)
        )
