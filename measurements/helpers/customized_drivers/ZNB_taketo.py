...
class FixedFrequencyPointDBPhase(MultiParameter):
    """
    Parameter for sweep that returns the magnitude of mean of the real (I) and
    imaginary (Q) parts of the VNA response and it's phase.
    Requires the use of the sweep type to be set to continuous wave mode.
    ...
    def get_raw(self) -> tuple[float, ...]:
        ...
        i, q = self.instrument._get_cw_data()
        s = np.mean(i) + 1j * np.mean(q)
        return float(20 * np.log10(float(np.abs(s)))), float(np.angle(s))
...
class FrequencySweepDBPhase(MultiParameter):
    ...
    def get_raw(self) -> tuple[ParamRawDataType, ...]:
        ...
        with self.instrument.format.set_to("Complex"):
            data = self.instrument._get_sweep_data(force_polar=True)
        return 20 * np.log10(np.abs(data)), np.unwrap(
            np.angle(data)
        )  ## editted by Taketo
...
class RohdeSchwarzZNBChannel(InstrumentChannel):
    def __init__(
        ...
        **kwargs: "Unpack[InstrumentBaseKWArgs]",
        ...
        self._model_min_source_power = {
            ...
            "ZNB20": -60,
            "ZNB40": -60,
            "ZNA26": -60,  # added by Taketo for AQUA lab VNA
        }
        ...
        self.vna_parameter: Parameter = self.add_parameter(
            name="vna_parameter",
            label="VNA parameter",
            get_cmd=f"CALC{self._instrument_channel}:PAR:MEAS? " f"'{self._tracename}'",
        ...
        ######################################################################
        # Added
        self.point_fixed_frequency_db_phase: FixedFrequencyPointDBPhase = (
            self.add_parameter(
                name="point_fixed_frequency_db_phase",
                parameter_class=FixedFrequencyPointDBPhase,
            )
        )
        """Parameter point_fixed_frequency_db_phase"""
        ######################################################################
    ...
    # stop = self.stop()
    # if val > stop: # val >= stop: !!!!!! Changed by Taketo because VNA can do npts=1 scan.
    ...
    # channel = self._instrument_channel
    # start = self.start()
    # if val < start: # val <= start: !!!!!! Changed by Taketo because VNA can do npts=1 scan.
    ...
    def _get_cw_data(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        with self.status.set_to(1):
            with self.root_instrument.timeout.set_to(self._get_timeout()):
                self.write(f"INIT{self._instrument_channel}:IMM; *WAI")
                data_str = self.ask(f"CALC{self._instrument_channel}:DATA? " f"SDAT")
            data = np.array(data_str.rstrip().split(",")).astype("float64")
            i = data[0::2]
            q = data[1::2]
    ...
    def _get_timeout(self) -> float:
        ...
        timeout = max(self.sweep_time.cache.get() * 5.0, timeout)  # editted by Taketo
        return timeout
    ...
    def __init__(
        ...
        **kwargs: "Unpack[VisaInstrumentKWArgs]",
        ...
        m_frequency = {
            ...
            "ZNB20": (100e3, 20e9),
            "ZNB40": (10e6, 40e9),
            "ZNA26": (10e6, 26.5e9),  # added by Taketo for AQUA lab VNA
        }
