...
param_dict = {
    ...
    "ro_pulse_length": 12e-6,  # [sec]
    "ro_power": 0,  # -22, # [dBm] ZInstrument output power
    "ro_acquire_range": -20,
    ...
    "ef_drive_power": 9.5,  # maybe need to be lower than "qu_power"(second tone). need to be fixed.
    "current": 0e-3,  # [A]
    # pulsed onetone
    "pulsed_onetone": {
        "ro_freq_step": 1e6,  # [Hz]
        ...
        "sweep": "ro_power",  # "current", # str. name of the sweep parameter
    ...
    "pulsed_twotone": {
        ...
        # "qu_lo_freq":6.4e9, #[GHz]
        "qu_power": -10,
        "qu_pi_pulse_length": 4000e-9,  # [sec]
        "reset_delay": 0.1e-6,  # [sec] or False (the same parameter in the higher nest will be used)
        "avg": 2
    ...
    "time_rabi": {
        ...
        "qu_pulse_length_stop": 1200e-9,
        "qu_pulse_length_npts": 30,
        "sweep": False,  # "current", # str. name of the sweep parameter
        "qu_power": 3.9 - 6,
        # "qu_freq":qu_freq+10e6,
        "reset_delay": 3 * T1,  # > 3*T1
        "avg": 2
        ** 11,  # [#] or False (the same parameter in the higher nest will be used)
        "external_avg": 1,  # [#] or False (the same parameter in the higher nest will be used)
    },
    # amp rabi
    "amp_rabi": {
    ...
    "T1": {
        ...
        "sweep": False,  # "current", # str. name of the sweep parameter
        # "ro_power":-5, # [dBm] ZInstrument output power
        "reset_delay": 5.1
    ...
    "ramsey": {
        ...
        "interval_sweep_stop": 60e-6,
        "interval_sweep_npts": 60,
    ...
    "echo": {
        ...
        "interval_sweep_list": np.hstack(
            [np.linspace(0, T2, 20), logsweep(T2 * 1.1, 5 * T2, 11)]
        ),
        "sweep": False,  # str. name of the sweep parameter
        "pi_pulse_rep": 5,  # N pi pulses in T2 Echo measurement
        "reset_delay": 5.1
        * T1,  # [sec] or False (the same parameter in the higher nest will be used)
        "avg": 2
        ** 12,  # [#] or False (the same parameter in the higher nest will be used)
    ...
    "qu_temp_amp_sweep": {
        ...
        "ef_pulse_amp_stop": 0.95,
        "ef_pulse_amp_npts": 16,
        "ef_pulse_length": 150e-9,
        "index_num": 1000,
        "ro_freq": ro_freq_e,
        "reset_delay": 3 * T1,  # > 3*T1
        "avg": 2
    ...
    "single_shot": {
        "n_shots": 2**14,
        "sweep": "ro_pulse_length",
        # "ro_pulse_length":10e-6, #[sec]
        "reset_delay": 3 * T1,  # > 3*T1
        "external_avg": 1,  # [#] or False (the same parameter in the higher nest will be used)
    },
    # pulse train
    ...
    "pulsed_acStark_shift": {
        "qu_freq_step": 0.2e6,  # [Hz]
        "qu_freq_start": 6.475e9,  # [Hz]
        "qu_freq_stop": 6.495e9,  # [Hz] must be larger than start
        "qu_freq_npts": False,  # if "False", npts is calculated using ro_freqstep
        "sweep": "cav_drive_power",  # str. name of the sweep parameter
        # "qu_power": -10,
        # "qu_pi_pulse_length":800e-9, #[sec]
        "reset_delay": 3
        * T1,  # [sec] or False (the same parameter in the higher nest will be used)
        "avg": 2
        ** 11,  # [#] or False (the same parameter in the higher nest will be used)
    ...
    # only for pulsed_acStark_shift
    # "cav_drive_freq":None,
    "cav_drive_power": -30,
    "cav_drive_length": 12e-6,
    "index": False,
    "qu_freq_detune": -0.05e6,  # -0.022e6, #[Hz] used only for ramsey measurement
    ...
    "sweep_stop": -30,
    "sweep_npts": 5,
    "sweep_list": False,  # [10*np.log10(p_mW) for p_mW in np.linspace(1e-3,10,80)],
    # "sweep_start":-2,
    # "sweep_stop":-2.5,
    # "sweep_npts":11,
    # "sweep_list":False,
