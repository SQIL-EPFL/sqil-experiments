...
exp_name = "interleaved_T1_ramsey"
tags = ["0_interleaved_T1_ramsey"]

T1_delay_list = param_dict["interleaved_T1_ramsey"]["delay_sweep_list"]

echo_interval_list = param_dict["interleaved_ramsey"]["interval_sweep_list"]

# update param_dict when local parameter used
local_param_list = param_dict["interleaved_ramsey"].keys()
for key in local_param_list:
    if key in param_dict.keys():
        if not param_dict["interleaved_ramsey"][key] == False:
            param_dict[key] = param_dict["interleaved_ramsey"][key]
...
with DDH5Writer(datadict, db_path_local, name=exp_name) as writer:
    ...
    for index in range(param_dict["interleaved_ramsey"]["index_num"]):
        if (
            param_dict["interleaved_ramsey"]["index_num"] == 1
            and param_dict["save_pulsesheet"] == True
        ):
            show_pulse_sheet(
                f"{writer.filepath.parent}/interleaved_ramsey_T1_echo_pulsesheet",
                compiled_exp,
                interactive=False,
            )
...
fig_indexed, fig_stats, T1, T2echo, Tphi, T1_std, T2echo_std, Tphi_std = (
    interleaved_ramsey_T1_echo_plot(
        index_list, T1_delay, T1_data, echo_interval_delay, echo_data, sigma_filter=True
    )
)

fig_indexed.savefig(
    f"{filepath_parent}/interleaved_ramsey_measurement.png", bbox_inches="tight"
)
fig_stats.savefig(
    f"{filepath_parent}/interleaved_ramsey_measurement_stats.png", bbox_inches="tight"
)
