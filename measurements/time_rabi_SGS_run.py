...
import inspection_only_optm
from inspection import inspect_decaying_oscillations
from time_rabi_SGS_exp import exp_file, main_exp

exp_name = "time_rabi_SGS"
tags = ["0_time rabi_SGS"]

# define pulse length list
qu_pulse_length_start = param_dict["time_rabi"]["qu_pulse_length_start"]
qu_pulse_length_stop = param_dict["time_rabi"]["qu_pulse_length_stop"]
if param_dict["time_rabi"]["qu_pulse_length_npts"] == False:
    qu_pulse_length_npts = (
        int(
            (qu_pulse_length_stop - qu_pulse_length_start)
            / param_dict["time_rabi"]["qu_pulse_length_step"]
        )
        + 1
    )
else:
    qu_pulse_length_npts = param_dict["time_rabi"]["qu_pulse_length_npts"]
param_dict["time_rabi"][
    "qu_pulse_length_npts"
] = qu_pulse_length_npts  # update qu_pulse_length_npts to param_dict
qu_pulse_length_sweep = np.linspace(
    qu_pulse_length_start, qu_pulse_length_stop, qu_pulse_length_npts
)

# update param_dict when local parameter used
local_param_list = param_dict["time_rabi"].keys()
for key in local_param_list:
    if key in param_dict.keys():
        if not param_dict["time_rabi"][key] == False:
            param_dict[key] = param_dict["time_rabi"][key]

if param_dict["time_rabi"]["sweep"] == False:
    sweep_list = [0]
    ...
    exp_name = exp_name + "_vs_" + param_dict["time_rabi"]["sweep"]
    tags.append(f"0_{exp_name}")
    param_dict[param_dict["time_rabi"]["sweep"]] = "sweeping"
...
with DDH5Writer(datadict, db_path_local, name=exp_name) as writer:
    ...
    for sweep_param in tqdm.tqdm(sweep_list):
        # update param_dict
        if not param_dict["time_rabi"]["sweep"] == False:
            param_dict[param_dict["time_rabi"]["sweep"]] = sweep_param
        ...
        # setting of SGS100A
        sgsa.power(param_dict["SGS_power"])
...
if param_dict["time_rabi"]["sweep"] == False:
    ...
    plt.savefig(f"{path}/time_rabi_inspect.png", bbox_inches="tight")
    inspection_only_optm.inspect_decaying_oscillations(
        qu_pulse_length_sweep, data, figure=None
    )
    plt.savefig(f"{path}/00_time_rabi_inspect.png", bbox_inches="tight")
