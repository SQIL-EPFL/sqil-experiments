...
from amp_rabi_exp import exp_file, main_exp
from inspection import inspect_decaying_oscillations_amprabi

exp_name = "amp_rabi"
tags = ["0_amp rabi"]

# define pulse amp list
qu_pulse_amp_start = param_dict["amp_rabi"]["qu_pulse_amp_start"]
qu_pulse_amp_stop = param_dict["amp_rabi"]["qu_pulse_amp_stop"]
if param_dict["amp_rabi"]["qu_pulse_amp_npts"] == False:
    qu_pulse_amp_npts = (
        int(
            (qu_pulse_amp_stop - qu_pulse_amp_start)
            / param_dict["amp_rabi"]["qu_pulse_amp_step"]
        )
        + 1
    )
else:
    qu_pulse_amp_npts = param_dict["amp_rabi"]["qu_pulse_amp_npts"]
param_dict["amp_rabi"][
    "qu_pulse_amp_npts"
] = qu_pulse_amp_npts  # update qu_pulse_amp_npts to param_dict
qu_pulse_amp_sweep = np.linspace(
    qu_pulse_amp_start, qu_pulse_amp_stop, qu_pulse_amp_npts
)

# update param_dict when local parameter used
local_param_list = param_dict["amp_rabi"].keys()
for key in local_param_list:
    if key in param_dict.keys():
        if not param_dict["amp_rabi"][key] == False:
            param_dict[key] = param_dict["amp_rabi"][key]


if param_dict["amp_rabi"]["sweep"] == False:
    sweep_list = [0]
    # define DataDict for saving in DDH5 format
    datadict = DataDict(qu_pulse_amp=dict(unit=""), data=dict(axes=["qu_pulse_amp"]))
    datadict.validate()
else:
    exp_name = exp_name + "_vs_" + param_dict["amp_rabi"]["sweep"]
    tags.append(f"0_{exp_name}")
    param_dict[param_dict["amp_rabi"]["sweep"]] = "sweeping"
...
with DDH5Writer(datadict, db_path_local, name=exp_name) as writer:
    ...
    for sweep_param in tqdm.tqdm(sweep_list):
        # update param_dict
        if not param_dict["amp_rabi"]["sweep"] == False:
            param_dict[param_dict["amp_rabi"]["sweep"]] = sweep_param
