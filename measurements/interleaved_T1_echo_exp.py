"""@author Taketo
Experiment code for interleaved T1 Echo-CPMG measurement
"""
...
def main_exp(session, param_dict):
    ...
    T1_delay_sweep = SweepParameter(
        uid="delay_sweep",
        values=param_dict["interleaved"]["delay_sweep_list"],
        axis_name="Readout delay [sec]",
    )
    # define an ramsey delay sweep
    echo_interval_sweep = SweepParameter(
        uid="interval_sweep",
        values=param_dict["interleaved"]["interval_sweep_list"],
    ...
    with exp.acquire_loop_rt(
        ...
        with exp.sweep(
            ...
            with exp.section(
                ...
                for _ in range(rep):
                    exp.delay(signal="qu_drive", time=echo_interval_sweep / 2 / rep)
                    exp.play(signal="qu_drive", pulse=qu_pi_pulse)
                    exp.delay(signal="qu_drive", time=echo_interval_sweep / 2 / rep)
                exp.play(signal="qu_drive", pulse=qu_half_pi_pulse)
