...
from echo_cpmg_exp import exp_file, main_exp
from inspection import inspect_decaying_exponent

exp_name = "Echo_CPMG"
tags = ["0_Echo", "0_CPMG", "T2"]

if param_dict["echo"]["logsweep"] == True:
    interval_sweep = param_dict["echo"]["interval_sweep_list"]
else:
    interval_sweep = np.linspace(
        param_dict["echo"]["interval_sweep_start"],
        param_dict["echo"]["interval_sweep_stop"],
        param_dict["echo"]["interval_sweep_npts"],
    )

# update param_dict when local parameter used
local_param_list = param_dict["echo"].keys()
for key in local_param_list:
    if key in param_dict.keys():
        if not param_dict["echo"][key] == False:
            param_dict[key] = param_dict["echo"][key]

if param_dict["echo"]["sweep"] == False:
    sweep_list = [0]
    # define DataDict for saving in DDH5 format
    datadict = DataDict(interval=dict(unit="sec"), data=dict(axes=["interval"]))
    datadict.validate()
else:
    exp_name = exp_name + "_vs_" + param_dict["echo"]["sweep"]
    tags.append(f"0_{exp_name}")
    param_dict[param_dict["echo"]["sweep"]] = "sweeping"
...
with DDH5Writer(datadict, db_path_local, name=exp_name) as writer:
    ...
    for sweep_param in tqdm.tqdm(sweep_list):
        # update param_dict
        if not param_dict["echo"]["sweep"] == False:
            param_dict[param_dict["echo"]["sweep"]] = sweep_param
...
if param_dict["echo"]["sweep"] == False:
    ...
    inspection_only_optm.inspect_decaying_exponent(interval_sweep, data, figure=None)
    plt.savefig(f"{writer.filepath.parent}/00_echo_cpmg.png", bbox_inches="tight")
