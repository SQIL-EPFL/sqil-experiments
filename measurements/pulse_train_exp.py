...
def main_exp(session, param_dict):
    ...
    ###############################################################################
    reps = LinearSweepParameter(
        uid="reps_sweep",
        start=param_dict["pulse_train"]["reps_sweep_start"],
        stop=param_dict["pulse_train"]["reps_sweep_stop"],
        count=param_dict["pulse_train"]["reps_sweep_npts"],
        axis_name="Pi pulse reps",
    )
    #####################################################################
    ...
    with exp.acquire_loop_rt(
        ...
        with exp.sweep(
            ...
            with exp.section(
                ...
                with exp.match(sweep_parameter=reps):
