...
def main_exp(session, param_dict):
    ...
    cal["qu_drive_ef"] = SignalCalibration(
        ...
        # range = ef_power_range # the same output port must use the same power range
    )
    ...
    interval_sweep = LinearSweepParameter(
        uid="interval_sweep",
        start=param_dict["ramsey_ef"]["interval_sweep_start"],
        stop=param_dict["ramsey_ef"]["interval_sweep_stop"],
        count=param_dict["ramsey_ef"]["interval_sweep_npts"],
    ...
    with exp.acquire_loop_rt(
        ...
        with exp.sweep(
            ...
            with exp.section(
                ...
                exp.play(signal="qu_drive_ef", pulse=ef_half_pi_pulse)
                exp.delay(signal="qu_drive_ef", time=interval_sweep)
                exp.play(signal="qu_drive_ef", pulse=ef_half_pi_pulse)
            with exp.section(
                uid="ge_excitation2",
                alignment=SectionAlignment.RIGHT,
            ...
            # measurement
            with exp.section(uid="readout", play_after="ge_excitation2"):
                exp.measure(
