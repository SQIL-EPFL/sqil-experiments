"""
20241031 created.
Pulsed threetone measurement.
Use SHFQC and Signal core for readout pulse (upconversion) and SHFQC for driving transmon.
No flux control.
"""
...
from three_tone_exp import exp_file, main_exp

exp_name = "pulsed_threetone"
tags = ["0_pulsed_threetone"]

# define readout frequency list
freq_start = param_dict["pulsed_threetone"]["ef_freq_start"]
freq_stop = param_dict["pulsed_threetone"]["ef_freq_stop"]
if param_dict["pulsed_threetone"]["ef_freq_npts"] == False:
    freq_npts = (
        np.abs(
            int(
                (freq_stop - freq_start)
                / param_dict["pulsed_threetone"]["ef_freq_step"]
            )
        )
        + 1
    )
else:
    freq_npts = param_dict["pulsed_threetone"]["ef_freq_npts"]
param_dict["pulsed_threetone"][
    "ef_freq_npts"
...
local_param_list = param_dict["pulsed_threetone"].keys()
for key in local_param_list:
    if key in param_dict.keys():
        if not param_dict["pulsed_threetone"][key] == False:
            param_dict[key] = param_dict["pulsed_threetone"][key]

# define datadict
if param_dict["pulsed_threetone"]["sweep"] == False:
    sweep_list = [0]
    ...
    exp_name = exp_name + "_vs_" + param_dict["pulsed_threetone"]["sweep"]
    tags.append(f"0_{exp_name}")
    param_dict[param_dict["pulsed_threetone"]["sweep"]] = "sweeping"
...
with DDH5Writer(datadict, db_path_local, name=exp_name) as writer:
    ...
    for sweep_param in tqdm.tqdm(sweep_list):
        # update param_dict
        if not param_dict["pulsed_threetone"]["sweep"] == False:
            param_dict[param_dict["pulsed_threetone"]["sweep"]] = sweep_param
    ...
    if param_dict["pulsed_threetone"]["sweep"] == False:
        fig_fit, f0, fwhm = spectroscopy_1D_plot(freq_list, data)
        ro_power = param_dict["ro_power"]
        fig_fit.suptitle(f"RO Power: {ro_power}dBm")
        fig_fit.savefig(
            f"{writer.filepath.parent}/00_ro_power_{ro_power}dBm_fit.png",
            bbox_inches="tight",
        )
        writer.save_text(
