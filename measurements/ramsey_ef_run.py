...
from ramsey_ef_exp import exp_file, main_exp
...
interval_sweep = np.linspace(
    param_dict["ramsey_ef"]["interval_sweep_start"],
    param_dict["ramsey_ef"]["interval_sweep_stop"],
    param_dict["ramsey_ef"]["interval_sweep_npts"],
)

# update param_dict when local parameter used
local_param_list = param_dict["ramsey_ef"].keys()
for key in local_param_list:
    if key in param_dict.keys():
        if not param_dict["ramsey_ef"][key] == False:
            param_dict[key] = param_dict["ramsey_ef"][key]

if param_dict["ramsey_ef"]["sweep"] == False:
    sweep_list = [0]
    # define DataDict for saving in DDH5 format
    datadict = DataDict(interval=dict(unit="sec"), data=dict(axes=["interval"]))
    datadict.validate()
else:
    exp_name = exp_name + "_vs_" + param_dict["ramsey_ef"]["sweep"]
    tags.append(f"0_{exp_name}")
    param_dict[param_dict["ramsey_ef"]["sweep"]] = "sweeping"
...
with DDH5Writer(datadict, db_path_local, name=exp_name) as writer:
    ...
    for sweep_param in tqdm.tqdm(sweep_list):
        # update param_dict
        if not param_dict["ramsey_ef"]["sweep"] == False:
            param_dict[param_dict["ramsey_ef"]["sweep"]] = sweep_param
...
if param_dict["ramsey_ef"]["sweep"] == False:
    ...
    plt.savefig(f"{writer.filepath.parent}/ramsey_ef_inspect.png", bbox_inches="tight")
