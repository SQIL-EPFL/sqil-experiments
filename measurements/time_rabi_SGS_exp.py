...
import numpy as np
from helpers.utilities import hdawg_power_calculator, shfqa_power_calculator
...
def main_exp(session, param_dict):
    ...
    qu_power_range, qu_signal_amp = hdawg_power_calculator(param_dict["qu_power"])
    qu_pulse_length_sweep = LinearSweepParameter(
        uid="qu_pulse_length_sweep",
        start=param_dict["time_rabi"]["qu_pulse_length_start"],
        stop=param_dict["time_rabi"]["qu_pulse_length_stop"],
        count=param_dict["time_rabi"]["qu_pulse_length_npts"],
        axis_name="Pulse length [sec]",
    )
    # qu_drive_pulse = pulse_library.const(
    # uid="qu_pulse",
    ...
    qu_drive_pulse = pulse_library.gaussian_square(
        uid="qu_pulse",
        length=400e-9,  # we overwrite by sweep parameter
        amplitude=qu_signal_amp,
        can_compress=True,
    )
    # define an oscillator for the qubit drive pulse
    cal["qu_drive"] = SignalCalibration(
        oscillator=None, local_oscillator=None, range=qu_power_range
    )
        ...
        with exp.sweep(
            ...
            with exp.section(
                ...
                exp.play(
                    ...
                    length=qu_pulse_length_sweep + 2 * param_dict["sideslope_length"],
                    pulse_parameters={"width": qu_pulse_length_sweep},
                )
            # measurement
