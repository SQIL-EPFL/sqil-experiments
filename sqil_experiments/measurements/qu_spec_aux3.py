...
class QuSpecAux3(ExperimentHandler):
    exp_name = "qubit_spectroscopy_aux3"
    db_schema = {
    ...
    def analyze(self, path, *args, **kwargs):
        return qu_spec_aux3_analysis(path=path, **kwargs)
...
def qu_spec_aux3_analysis(
    ...
    **kwargs,
    ...
    if sweep_info[0].id == "aux_frequency":
        # print(x_data.shape)
        # print(y_data.shape)
        fig, axs = fig, axs = plt.subplots(2, 1, figsize=(20, 12), sharex=True)
        anal_res.add_figure(fig, "fig", qu_id)
        x_data_scaled = sweeps[0] * sweep_info[0].scale
        y_data_scaled = np.average(y_data, axis=1) * y_info.scale
